...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  const { jobs, addJob, updateJob, deleteJob, resetJobs } = useJobs();
  const { blogs, addBlog, updateBlog, deleteBlog, resetBlogs } = useBlogs();
  ...
  const handleSaveJob = (job: Omit<JobListing, 'id'>) => {
    if (editingJob) {
  ...
  const handleSaveBlog = (blog: Omit<BlogPost, 'id'>) => {
    if (editingBlog) {
      updateBlog(editingBlog.id, blog);
        ...
        {activeTab === 'overview' && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              <StatCard label="Total Jobs" value={jobs.length} icon={<Briefcase className="h-5 w-5" />} color="teal" />
              <StatCard label="Total Blogs" value={blogs.length} icon={<FileText className="h-5 w-5" />} color="cyan" />
              <StatCard label="Categories" value={new Set([...jobs.map(j => j.category), ...blogs.map(b => b.category)]).size} icon={<Tag className="h-5 w-5" />} color="emerald" />
              <StatCard label="Published" value={jobs.length + blogs.length} icon={<Sparkles className="h-5 w-5" />} color="blue" />
            </div>
        ...
        {activeTab === 'blogs' && (
          <div className="space-y-6">
            <ToolBar
              ...
              onReset={() => { resetBlogs(); showToast('Blogs reset to default demo data.'); }}
            />
...
function StatCard({ label, value, icon, color }: { label: string; value: number; icon: React.ReactNode; color: 'teal' | 'cyan' | 'emerald' | 'blue' }) {
  const colors = {
  ...
  return (
    <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
      ...
      <span className="block text-3xl font-extrabold text-slate-950 tabular-nums">{value}</span>
    </div>
  );
}
