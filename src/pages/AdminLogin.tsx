...
import { useAdminAuth, ADMIN_CREDENTIALS } from '../hooks/useDataStore';
import { Lock, User, Eye, EyeOff, Sparkles, ShieldCheck, Copy, Check, AlertCircle } from 'lucide-react';
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (ok) {
        ...
        setError('Invalid username or password. Use the credentials shown below.');
      }
    }, 500);
  };
      ...
      <div className="relative z-10 mx-auto flex max-w-6xl flex-col items-center justify-center px-4 sm:px-6 lg:px-8">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-600 max-w-md mx-auto">
            Sign in to manage job listings and blog posts published across the IVA Work Solutions website.
          </p>
        </div>
        ...
        <div className="grid w-full max-w-4xl grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="relative overflow-hidden rounded-3xl border-2 border-dashed border-[#5EE3B7]/40 bg-gradient-to-br from-[#5EE3B7]/5 to-[#00BFEF]/5 p-8">
            <div className="space-y-5">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#5EE3B7]/20 to-[#00BFEF]/20 border border-[#5EE3B7]/30">
                  <Sparkles className="h-5 w-5 text-[#00BFEF]" />
                </div>
                <div>
                  <h3 className="text-sm font-extrabold text-slate-950">Demo Credentials</h3>
                  <p className="text-[11px] text-slate-500">Click to copy or auto-fill the form</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="rounded-xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Username</span>
                      <p className="mt-0.5 font-mono text-sm font-bold text-slate-900">{ADMIN_CREDENTIALS.username}</p>
                    </div>
                    <button
                      type="button"
                      onClick={() => copyToClipboard(ADMIN_CREDENTIALS.username, 'username')}
                      className="flex h-9 w-9 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition-all hover:bg-slate-200"
                      title="Copy username"
                    >
                      {copied === 'username' ? <Check className="h-4 w-4 text-emerald-600" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-2">
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Password</span>
                      <p className="mt-0.5 font-mono text-sm font-bold text-slate-900">{ADMIN_CREDENTIALS.password}</p>
                    </div>
                    <button
                      type="button"
                      ...
                      title="Copy password"
                    >
                      {copied === 'password' ? <Check className="h-4 w-4 text-emerald-600" /> : <Copy className="h-3.5 w-3.5" />}
                    </button>
                  </div>
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={fillDemo}
                className="w-full rounded-xl border border-[#5EE3B7]/40 bg-white py-2.5 text-xs font-bold text-[#00BFEF] transition-all hover:bg-[#5EE3B7]/10"
              >
                Auto-fill Credentials
              </button>

              <div className="rounded-xl bg-white/70 border border-slate-100 p-3 text-[11px] leading-relaxed text-slate-600">
                <strong className="font-bold text-slate-700">Note:</strong> This is a demo environment using browser localStorage. All changes are saved locally on this device only.
              </div>
            </div>
          </div>
        </div>
